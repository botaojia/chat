...
public:
	...
	void onMessage(std::array<char, MAX_IP_PACK_SIZE>& msg) {
		...
		{
			std::cout << "[" << std::this_thread::get_id() << "]" << " onMessage, async_write :"
...
private:
	...
	void readHandler(const boost::system::error_code& error)
	{
		...
		std::cout << " handle_read write_msgs_.size() " << write_msgs_.size() << std::endl;
	...
	{
		std::cout << " handle_write write_msgs_.size() " << write_msgs_.size() << std::endl;
		if (!error)
		{
			write_msgs_.pop_front();
			std::cout << "[" << std::this_thread::get_id() << "]" << " handle_write, write_msgs_.size()=" << write_msgs_.size() << std::endl;

			if (!write_msgs_.empty())
			{
				std::cout << "[" << std::this_thread::get_id() << "]" << " handle_write, async_write " << write_msgs_.front().data() << std::endl;
				boost::asio::async_write(socket_,
						                 boost::asio::buffer(write_msgs_.front(), write_msgs_.front().size()),
						                 strand_.wrap(boost::bind(&personInRoom::writeHandler, shared_from_this(), _1))
										 );
		...
		{
			std::cout << "[" << std::this_thread::get_id() << "]" << " handle_write, leave room " << std::endl;
			room_.leave(shared_from_this());
		}
	}
