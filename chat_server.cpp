...
public:
	...
	void broadcast(std::array<char, MAX_IP_PACK_SIZE>& msg,
		...
		std::for_each(participants_.begin(), participants_.end(),
				boost::bind(&participant::onMessage, _1,
						std::ref(formatted_msg)));
	}
...
class personInRoom: public participant, public std::enable_shared_from_this<
		personInRoom> {
public:
	...
	void onMessage(std::array<char, MAX_IP_PACK_SIZE>& msg) {
		...
		if (!write_in_progress) {
			std::cout << "[" << std::this_thread::get_id() << "]"
					<< " onMessage, async_write :" << write_msgs_.front().data()
					<< std::endl;
			boost::asio::async_write(socket_,
					boost::asio::buffer(write_msgs_.front(),
							write_msgs_.front().size()),
					strand_.wrap(
							boost::bind(&personInRoom::writeHandler,
									shared_from_this(), _1)));
		}
	}
...
private:
	void nicknameHandler(const boost::system::error_code& error) {
		...
		std::cout << nickname_.data() << std::endl;
		room_.enter(shared_from_this(), std::string(nickname_.data()));
		boost::asio::async_read(socket_,
				boost::asio::buffer(read_msg_, read_msg_.size()),
				strand_.wrap(
						boost::bind(&personInRoom::readHandler,
								shared_from_this(), _1)));
	}

	void readHandler(const boost::system::error_code& error) {
		std::cout << "chat_room handle_read " << read_msg_.data() << std::endl;
		std::cout << " handle_read write_msgs_.size() " << write_msgs_.size()
				<< std::endl;
	...
	void writeHandler(const boost::system::error_code& error) {
		std::cout << " handle_write write_msgs_.size() " << write_msgs_.size()
				<< std::endl;
		if (!error) {
			write_msgs_.pop_front();
			std::cout << "[" << std::this_thread::get_id() << "]"
					<< " handle_write, write_msgs_.size()="
					<< write_msgs_.size() << std::endl;

			if (!write_msgs_.empty()) {
				std::cout << "[" << std::this_thread::get_id() << "]"
						<< " handle_write, async_write "
						<< write_msgs_.front().data() << std::endl;
				boost::asio::async_write(socket_,
						boost::asio::buffer(write_msgs_.front(),
								write_msgs_.front().size()),
						strand_.wrap(
			...
			std::cout << "[" << std::this_thread::get_id() << "]"
					<< " handle_write, leave room " << std::endl;
			room_.leave(shared_from_this());
		}
	}
...
public:
	server(boost::asio::io_service& io_service,
			...
			io_service_(io_service), strand_(strand), acceptor_(io_service,
					endpoint) {
		run();
	}
...
private:
	...
	void run() {
		...
		acceptor_.async_accept(new_participant->socket(),
				strand_.wrap(
						boost::bind(&server::onAccept, this, new_participant,
								_1)));
	}
